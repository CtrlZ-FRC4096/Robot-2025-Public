...
def flip_Rotation2d(self, rotation):
    return rotation.rotateBy(Rotation2d.fromDegrees(180)) #if self.shouldFlip else rotation
def flip_Pose2d(self, pose):
    return Pose2d(self.flip_Translation2d(pose.translation()), self.flip_Rotation2d(pose.rotation())) #if self.shouldFlip else pose
...
class FieldConstants():
    ...
    def flip_Translation2d (translation):
        return Translation2d(FieldConstants.flip_X_coord(translation.X()), FieldConstants.flip_Y_coord(translation.Y())) #if self.shouldFlip else translation
    @staticmethod
    def flip_Rotation2d(rotation):
        return rotation.rotateBy(Rotation2d.fromDegrees(180)) #if self.shouldFlip else rotation
    @staticmethod
    def flip_Pose2d(pose):
        return Pose2d(FieldConstants.flip_Translation2d(pose.translation()), FieldConstants.flip_Rotation2d(pose.rotation())) #if self.shouldFlip else pose
