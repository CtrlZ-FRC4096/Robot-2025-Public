...
class AutoRoutines:
    ...
    def three_piece_auto(self):
        return SequentialCommandGroup(
            ...
            lambda: self.robot.coroutines.score_piece.until(lambda: not self.robot.has_coral).withTimeout(5.0),
            lambda: self.robot.coroutines.reset_robot_after_scoring,
            self.robot.followPathCommand("3P_2"),
            lambda: self.robot.coroutines.intake_coral.until(lambda: self.robot.has_coral),
            lambda: self.robot.coroutines.reset_robot_after_intaking,
