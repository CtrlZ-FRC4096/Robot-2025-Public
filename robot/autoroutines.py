...
class AutoRoutines:
    ...
    def three_piece_auto(self):
        return SequentialCommandGroup(
            ...
            autoroutine2command(self.robot.coroutines.score_L4()),  
            autoroutine2command(self.robot.coroutines.score_left_branch()),
            autoroutine2command(self.robot.coroutines.score_piece()),
            autoroutine2command(self.robot.coroutines.reset_robot_after_scoring()),
            self.robot.followPathCommand("3P_2"),
            autoroutine2command(self.robot.coroutines.intake_coral().until(lambda: self.robot.has_coral).withTimeout(3.0)),
            self.robot.followPathCommand("3P_3"),
            autoroutine2command(self.robot.coroutines.score_L4()),
            autoroutine2command(self.robot.coroutines.score_right_branch()),
            autoroutine2command(self.robot.coroutines.score_piece()),
            autoroutine2command(self.robot.coroutines.reset_robot_after_scoring()),
