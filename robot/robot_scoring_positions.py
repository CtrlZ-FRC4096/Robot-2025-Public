class RobotScoringPositions:
	elevator_raise_threshold = 1.0 # meters needed to be closer than to reef to raise elevator
	end_effector_travel_position = 8.0
	elevator_intake_height = 1.5
	end_effector_intake_position = 0.0
	class L1_Scoring:
		elevator_height = 7.18
		end_effector_outtake_speed = 67
		end_effector_position = 10.0
		number = 1
	class L2_Scoring:
		elevator_height = 21.0
		end_effector_outtake_speed = 55
		end_effector_position = 10.0
		number = 2
	class L3_Scoring:
		elevator_height = 35.8
		end_effector_outtake_speed = 57
		end_effector_position = 10.0
		number = 3
	class L4_Scoring:
		elevator_height = 61.0
		end_effector_outtake_speed = 75
		end_effector_position = 10.0
		number = 4